...
File = open(MyDir + '/' + VChost + '_info.csv','w')
File.write("DataCenter;Cluster;HostName;Manufacture;Serial Number;ESXi Version;CPU Model;CPU Core;Memory;NICs;CPUUsage;MemUsage;Status\n")
print "DataCenter;Cluster;HostName;Manufacture;Serial Number;ESXi Version;CPU Model;CPU Core;Memory;NICs;CPUUsage;MemUsage;Status"
...
for dc_mor, dc_name in dcs.items():
  ...
  for c_mor, c_name in clusters.items():
	 ...
	 #  print "%s;%s" % (k,v)
         ...
         print "%s;%s;%s;%s;%s;%s;%s;%s(%s x %s);%.0fGB;%s;%s;%.0fGB;%s" % (dc_name,c_name,h_name,HostModel,HostModel,ESXiVer,CPUModel,TotalCPU,numCpuPackages,numCpuCores,TotalMem,myNICs,CPUUsage, MemUsage,MyStatus)
         myOP = "%s;%s;%s;%s;%s;%s;%s;%s(%s x %s);%.0fGB;%s;%s;%.0fGB;%s\n" % (dc_name,c_name,h_name,HostModel,HostModel,ESXiVer,CPUModel,TotalCPU,numCpuPackages,numCpuCores,TotalMem,myNICs,CPUUsage, MemUsage,MyStatus)
	 File.write(myOP)
