...
#############################
def usage():
  print "\n * Usage: "+sys.argv[0]+" {--verbose|--host <vcenter host> | --user <user> | --pass <password> --myvm <vmname> --action [RemoveNIC|AddNIC]}\n"
...
if MyAction == "ON":
  ...
  net_device = None
  for dev in vm_data.properties.config.hardware.device:
     if (dev._type in ["VirtualE1000", "VirtualE1000e", "VirtualPCNet32", "VirtualVmxnet", "VirtualVmxnet3"]):
        net_device = dev._obj
	#break
	#print hasattr(dev.backing, "deviceName")
	#print hasattr(dev, "backing")
	print dev.deviceInfo.label
	print dev.deviceInfo.summary
	print net_device.Connectable.Connected 
     #if not net_device:
        #server.disconnect()
        #raise Exception("The vm seems to lack a Virtual Nic")
  for v in vm_data.get_property("devices").values():
     print v
  for k,v in vm_data.get_property("devices").items():
     print k,v
else:
  usage()
